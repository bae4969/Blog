"""본문 HTML 정화 — **저장할 때와 보여줄 때 두 번** 한다(PHP 정책을 옮겨 왔다).

  · 저장할 때 — `sanitize_for_save` (PHP `Models/Post.php::create`)
  · 보여줄 때 — `sanitize` (PHP `Core/HtmlSanitizer.php`)

2026-09-25 부터 태그 목록은 둘이 같다(속성만 저장 쪽이 조금 좁다). 그래도 출력 정화를 없애면
안 된다 — DB 에는 **저장 정화를 안 거친 글**이 있다(옛 PHP 글, n8n 이 DB 에 직접 넣던 글).
"""
...
def sanitize_for_save(html: str | None) -> str:
    ...
    ⚠️ 저장 때 걸렀다고 출력 정화를 생략하면 안 된다 — 저장 정화를 안 거친 글이 있다(모듈 설명).
    """
    if not html:
        return ""
    cleaned = nh3.clean(
