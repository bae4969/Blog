...
return [
    ...
    // 신뢰할 수 있는 프록시 IP 목록 (이 IP에서만 X-Real-IP / X-Forwarded-For 헤더를 신뢰)
    // 172.16.9.0/24 = db_bridge 도커 게이트웨이 — nginx-proxy-manager가 호스트 포트로 프록시하면서
    // NAT를 거쳐 REMOTE_ADDR이 172.16.9.1로 바뀌므로, 여기를 신뢰해야 실제 방문자 IP를 복원할 수 있다
    'trusted_proxies' => ['127.0.0.1', '::1', '192.168.135.0/24', '172.16.9.0/24'],
    ...
    'ip_block' => [
        ...
        // 차단 제외 IP/CIDR 목록 (예: 192.168.0.0/24)
        // 172.16.9.1(도커 게이트웨이)은 프록시 헤더가 유실됐을 때만 나타나는 값 — 이때 차단하면
        // 외부 방문자 전원이 한 IP로 묶여 사이트 전체가 403이 되므로 차단 대상에서 제외한다
        'whitelist' => ['127.0.0.1', '::1', '192.168.135.0/24', '172.16.9.1'],
