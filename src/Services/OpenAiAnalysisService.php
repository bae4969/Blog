...
class OpenAiAnalysisService
{
    ...
    private const MODELS_LIST_ENDPOINT = 'https://api.openai.com/v1/models';
    ...
    public function analyze(array $videos, array $config): ?array
    {
        ...
        $response = $this->postJson(self::ENDPOINT, $body, $apiKey, $timeout);
        if ($response === null) {
            return null;
        }
    ...
    private function parseResponse(string $rawJson): ?array
    {
        ...
        $choices = $env['choices'] ?? null;
        if (!is_array($choices) || empty($choices)) {
            return null;
        }

        $inner = (string)($choices[0]['message']['content'] ?? '');
        if ($inner === '') {
            return null;
        }
