...
<div class="stock-admin-container">
    ...
    <div class="stock-admin-summary">
        <div class="stock-admin-card">
            ...
            <strong class="stock-admin-card-value"><?= number_format($totalCount) ?></strong>
        </div>
        <div class="stock-admin-card">
            <span class="stock-admin-card-label">현재 시장 등록 종목</span>
            <strong class="stock-admin-card-value" id="registeredCountLabel"><?= number_format($currentRegisteredCount) ?></strong>
        </div>
        <div class="stock-admin-card">
            <span class="stock-admin-card-label">현재 시장 선택 수</span>
            <strong class="stock-admin-card-value" id="selectedCountLabel">0</strong>
        </div>
    </div>
    ...
    <div class="stock-admin-toolbar">
        <div class="stock-admin-market-tabs">
            <?php foreach ($marketLabels as $marketCode => $marketLabel): ?>
                ...
                <a class="stock-admin-market-tab <?= $currentMarket === $marketCode ? 'active' : '' ?>" href="/stocks/admin?<?= http_build_query($marketQuery) ?>">
    ...
    <form class="stock-admin-table-form" method="post" action="/stocks/admin/subscriptions" id="stockAdminForm">
        ...
        <?php if ($totalPages > 1): ?>
            <div class="pagination">
                <?php if ($currentPage > 1): ?>
                    ...
                    <a class="page-btn" href="/stocks/admin?<?= http_build_query($previousQuery) ?>">&lt;</a>
                <?php endif; ?>
                ...
                ?>
                    <a class="page-btn <?= $pageNumber === $currentPage ? 'active' : '' ?>" href="/stocks/admin?<?= http_build_query($pageQuery) ?>">
                        <?= $pageNumber ?>
                    </a>
                <?php endfor; ?>
                ...
                <?php if ($currentPage < $totalPages): ?>
                    ...
                    <a class="page-btn" href="/stocks/admin?<?= http_build_query($nextQuery) ?>">&gt;</a>
                <?php endif; ?>
            </div>
        <?php endif; ?>
    </form>
