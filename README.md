# Developer Notes — 주식 대시보드 + 블로그

FastAPI 로 만든 주식·암호화폐 분석 화면과 개인 블로그입니다. **주식이 메인**이고(2026-09-23),
블로그는 곁에 둡니다 — 사이트 루트는 `/stocks` 로 가고 `blog.` 도메인만 `/blog` 로 갑니다.
인증은 직접 하지 않고 중앙 인증 서비스(`10.auth`)가 발급한 토큰을 검증만 합니다.
